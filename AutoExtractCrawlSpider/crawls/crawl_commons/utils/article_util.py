...
class ArticleUtils(object):
    ...
    FILE_POSTFIXS = [".pdf", ".doc", ".xls", ".xlsx", ".docx", ".pptx", ".ppt", ".PDF", ".DOC", ".XLS", ".XLSX",
                     ".DOCX", ".PPTX", ".PPT"]
    MERGE_FIELD = ["content", "contentImages", "contentFiles", "contentSnapshot"]
    PAGE_CONTENT = [">上页<", ">上一页<", ">下页<", ">下一页<", ">末页<", ">尾页<", ">首页<"]
    COMMON_NEXT_PAGE_REGEX = [WebRegex({"regexType": "xpath", "regexField": "nextPage",
                                        "regexContent": '//a[contains(text(),"下一页") or contains(text(),"下页") or contains(text(),"后一页") or text()=">" ]//@href',
                                        "resultFormat": "", "pageRenderType": 0, "renderSeconds": "0",
                                        "renderType": "0", "renderBrowser": "", "regexSort": "0", "depthNumber": "0",
                                        "resultFilterRegex": "", "maxPageNumber": "0"})]
    ...
    ERROR_PAGE_CONTENT_PATTERN = re.compile(
        u'.*?(页面已删除|请开启JavaScript|页面不存在|资源可能已被删除|请用新域名访问|BadGateway|BadRequest|ErrorPage).*')
    ...
    def getAutoDetail(cls, contentPageNumber, html, enableDownloadImage=False, enableSnapshot=False):
        ...
        try:
            ...
            if StringUtils.isNotEmpty(ArticleUtils.removeAllTag(contentSnapshot)):
                if enableSnapshot:
                    autoDetail["contentSnapshot"] = contentSnapshot.replace("<html>", "")\
    ...
    def isErrorPage(cls, detail):
        ...
        if StringUtils.isEmpty(contentRemoveTag) and (
                "contentImages" not in detail or StringUtils.isEmpty(detail["contentImages"])) and (
                "contentFiles" not in detail or StringUtils.isEmpty(detail["contentFiles"])):
            return True
